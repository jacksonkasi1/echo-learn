...
// ** import schema
import { confirmUploadSchema, type ConfirmUploadRequest } from '@/schema/upload'

// ** import utils
import { updateFileStatus, getFileMetadata } from '@/lib/upstash/redis'
...
confirmRoute.post(
  ...
  async (c: Context) => {
    try {
      const { fileId, userId } = c.req.valid('json' as never) as ConfirmUploadRequest
