// ** import types
import type { KnowledgeGraph, GraphNode, GraphEdge, GraphMergeResult } from '@repo/shared'

// ** import lib
import { redis, getKnowledgeGraph, saveKnowledgeGraph } from '@/lib/upstash/redis'
  ...
  try {
    ...
    for (const newNode of newGraph.nodes) {
      ...
      if (existingNode) {
        ...
        if (newNode.description && (!existingNode.description || newNode.description.length > existingNode.description.length)) {
  ...
  try {
    ...
    const updatedNodes = graph.nodes
      ...
      .filter((node) => {
        ...
        return (node.fileIds && node.fileIds.length > 0) || connectedNodes.has(node.id)
      })
...
export async function findRelatedNodes(
  ...
  try {
    ...
    const queue: Array<{ id: string; depth: number }> = [{ id: normalizedNodeId, depth: 0 }]
