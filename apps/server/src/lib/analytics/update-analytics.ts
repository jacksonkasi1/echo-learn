// ** import types
import type { InteractionLog, UserProfile } from '@repo/shared'
...
import { extractTopicsFromConversation } from '@/lib/llm/generate-response'
...
export async function updateLearningLevel(userId: string): Promise<void> {
  try {
    ...
    if (questionsAnswered >= 100 && strongCount > weakCount * 2 && totalTopics >= 20) {
      newLevel = 'expert'
    } else if (questionsAnswered >= 50 && strongCount > weakCount && totalTopics >= 10) {
      newLevel = 'advanced'
